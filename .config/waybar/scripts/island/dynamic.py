...
OUT = f"store/dynamic_out.txt"
# PREV_PATH = f"{Path.home()}/.config/waybar/store/prev.txt"
prev = None
...
# with open(f"{Path.home()}/.config/waybar/im_here","w") as f:
#     f.write("")
...
def notif_watcher():
    # with open(f"{Path.home()}/.config/waybar/store/latest_notif.txt","rb") as f:
    with open("store/latest_notif.txt","rb") as f:
        global prev
        try:
            new = pickle.load(f)
        except:
...
with open("store/test.log", "wb") as f:
